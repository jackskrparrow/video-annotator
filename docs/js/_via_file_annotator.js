...
_via_file_annotator.prototype._metadata_add = function(region_shape, canvas_input_pts) {
  return new Promise( function(ok_callback, err_callback) {
    ...
    this.d.metadata_add(this.vid, z, xy, av).then( function(ok) {
      // Add video source information for dual video mode
      var mid = ok.mid;
      if (this.d.store.view[this.vid].fid_list.length === 2) {
        // This is dual video mode, add video source to metadata
        var video_source = '';
        if (this.file_label === 'Video 1' || this.file_label === '') {
          video_source = 'V1';
        } else if (this.file_label === 'Video 2') {
          video_source = 'V2';
        }
        
        // Store video source in metadata flags or as a custom property
        if (video_source) {
          this.d.store.metadata[mid].video_source = video_source;
        }
      }
      ok_callback(mid);
    }.bind(this), function(err) {
...
_via_file_annotator.prototype._rinput_enable = function() {
  ...
  // Check if fid is defined before accessing file properties
  if (this.fid && this.d.store.file[this.fid]) {
    if ( this.d.store.file[this.fid].type === _VIA_FILE_TYPE.VIDEO ||
