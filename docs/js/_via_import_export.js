...
_via_import_export.prototype.export_to_coco = function() {
  return new Promise( function(ok_callback, err_callback) {
    ...
    // Sort metadata by temporal coordinates (z[0]) for chronological order
    var sorted_metadata = Object.keys(this.d.store['metadata']).sort(function(a, b) {
      var z_a = this.d.store['metadata'][a].z && this.d.store['metadata'][a].z.length > 0 ? this.d.store['metadata'][a].z[0] : Infinity;
      var z_b = this.d.store['metadata'][b].z && this.d.store['metadata'][b].z.length > 0 ? this.d.store['metadata'][b].z[0] : Infinity;
      return z_a - z_b;
    }.bind(this));
    
    for ( var i = 0; i < sorted_metadata.length; i++ ) {
      var mid = sorted_metadata[i];
...
_via_import_export.prototype.export_to_via3_csv = function() {
  return new Promise( function(ok_callback, err_callback) {
    ...
    csv.push('# CSV_HEADER = metadata_id,file_list,flags,temporal_coordinates,spatial_coordinates,metadata,video_source');
    ...
    for ( var i = 0; i < sorted_metadata.length; i++ ) {
      ...
      // Add video source identifier for dual video mode
      var video_source = '';
      if (this.d.store.metadata[mid].xy && this.d.store.metadata[mid].xy.length > 0) {
        // This is a spatial region annotation (video frame annotation)
        var vid = this.d.store.metadata[mid].vid;
        
        // Check if this is dual video mode by looking at the original view
        var original_vid = null;
        for (var vindex in this.d.store.project.vid_list) {
          var check_vid = this.d.store.project.vid_list[vindex];
          if (this.d.store.view[check_vid].fid_list.length === 2) {
            original_vid = check_vid;
            break;
          }
        }
        
        if (original_vid) {
          // This is dual video mode
          var original_fid_list = this.d.store.view[original_vid].fid_list;
          var current_fid_list = this.d.store.view[vid].fid_list;
          
          // Determine which video this annotation belongs to
          if (current_fid_list[0] === original_fid_list[0]) {
            video_source = 'V1';
          } else if (current_fid_list[0] === original_fid_list[1]) {
            video_source = 'V2';
          } else {
            // Fallback: check if metadata has video_source property
            if (this.d.store.metadata[mid].video_source) {
              video_source = this.d.store.metadata[mid].video_source;
            } else {
              // Try to determine from file name or other properties
              var file_name = this.d.store.file[current_fid_list[0]].fname;
              if (file_name.includes('1') || file_name.includes('first')) {
                video_source = 'V1';
              } else if (file_name.includes('2') || file_name.includes('second')) {
                video_source = 'V2';
              } else {
                // Additional fallback: check if this is a single-file view created for dual video mode
                // In dual video mode, each video gets its own view with a single file
                var is_dual_video_view = false;
                for (var check_vid in this.d.store.view) {
                  if (this.d.store.view[check_vid].fid_list.length === 2) {
                    is_dual_video_view = true;
                    break;
                  }
                }
                
                if (is_dual_video_view) {
                  // This is a dual video mode, determine by view ID or position
                  var vid_num = parseInt(vid);
                  if (vid_num % 2 === 0) {
                    video_source = 'V2';
                  } else {
                    video_source = 'V1';
                  ...
                  video_source = 'V1'; // Default to V1 for backward compatibility
                }
              }
            }
          }
          ...
          // Debug logging
          console.log('Dual video export - mid:', mid, 'vid:', vid, 'video_source:', video_source);
          console.log('Original fid_list:', original_fid_list, 'Current fid_list:', current_fid_list);
          console.log('All views:', Object.keys(this.d.store.view));
          console.log('All metadata:', Object.keys(this.d.store.metadata));
        }
      }
      line.push(video_source);
      csv.push(line.join(','));
    }
