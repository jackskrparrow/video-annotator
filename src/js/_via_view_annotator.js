...
_via_view_annotator.prototype._view_init = function(vid) {
  ...
  case 2:
    ...
    } else if ( this._view_has_only_video(vid) ) {
      this._view_annotate_two_videos(vid);
    } else {
      console.warn('View mode not supported yet!');
    }
    break;
...
_via_view_annotator.prototype._view_annotate_two_videos = function(vid) {
  this.view_mode = _VIA_VIEW_MODE.VIDEO2;
  ...
  this.file_annotator[0] = [];
  var fid0 = this.d.store.view[vid].fid_list[0];
  var fid1 = this.d.store.view[vid].fid_list[1];
  
  // Create temporary single-file views for each video
  var vid0 = this.d._view_get_new_id();
  this.d.store.view[vid0] = new _via_view([fid0]);
  
  var vid1 = this.d._view_get_new_id();
  this.d.store.view[vid1] = new _via_view([fid1]);
  
  // Create file annotators
  this.file_annotator[0][0] = new _via_file_annotator(this, this.d, vid0, 'Video 1', this.file_container[0][0]);
  this.file_annotator[0][1] = new _via_file_annotator(this, this.d, vid1, 'Video 2', this.file_container[0][1]);
  ...
  promise_list.push( this.file_annotator[0][1]._file_load() );
  Promise.all( promise_list ).then( function(ok) {
    // Setup dual video synchronization
    this._setup_dual_video_sync();
    
    // Setup temporal segmenter for dual video mode
    this.view_metadata_container.innerHTML = '';
    
    // Create temporal segmenter container
    this.temporal_segmenter_container = document.createElement('div');
    this.temporal_segmenter_container.classList.add('temporal_segmenter_container');
    this.view_metadata_container.appendChild(this.temporal_segmenter_container);
    
    // Create temporal segmenter for the first video (primary)
    this.temporal_segmenter = new _via_temporal_segmenter(this.file_annotator[0][0],
                                                          this.temporal_segmenter_container,
                                                          vid0,
                                                          this.d,
    ...
    // Setup view metadata editor
    this.vid_pair_annotator_container = document.createElement('div');
    this.vid_pair_annotator_container.classList.add('vid_pair_annotator_container');
    this.view_metadata_container.appendChild(this.vid_pair_annotator_container);
    this._metadata_update();
  }.bind(this), function(err) {
    console.warn('Failed to load videos');
    _via_util_msg_show('Failed to load videos!');
  }.bind(this));
}

// Add dual video synchronization functionality
_via_view_annotator.prototype._setup_dual_video_sync = function() {
  var video1 = this.file_annotator[0][0].file_html_element;
  var video2 = this.file_annotator[0][1].file_html_element;
  
  // Get video durations and find the shorter one
  var duration1 = video1.duration || 0;
  var duration2 = video2.duration || 0;
  this.sync_duration = Math.min(duration1, duration2);
  
  // --- 新增同步锁 ---
  this._syncing_seek = false;
  
  // Set up event listeners for video synchronization
  var self = this;
  
  // Remove existing event listeners to prevent duplicates
  if (this._video1_play_handler) {
    video1.removeEventListener('play', this._video1_play_handler);
    video1.removeEventListener('pause', this._video1_pause_handler);
    video1.removeEventListener('seeked', this._video1_seeked_handler);
  }
  if (this._video2_play_handler) {
    video2.removeEventListener('play', this._video2_play_handler);
    video2.removeEventListener('pause', this._video2_pause_handler);
    video2.removeEventListener('seeked', this._video2_seeked_handler);
  }
  
  // Create event handlers for key synchronization moments only
  this._video1_play_handler = function() {
    video2.currentTime = video1.currentTime;
    video2.play();
  };
  
  this._video1_pause_handler = function() {
    video2.pause();
  };
  
  this._video1_seeked_handler = function() {
    if (self._syncing_seek) return;
  ...
  this._video2_play_handler = function() {
    video1.currentTime = video2.currentTime;
    video1.play();
  };
  
  this._video2_pause_handler = function() {
    video1.pause();
  };
  
  this._video2_seeked_handler = function() {
    if (self._syncing_seek) return;
  ...
  // Add event listeners for key moments only
  video1.addEventListener('play', this._video1_play_handler);
  video1.addEventListener('pause', this._video1_pause_handler);
  video1.addEventListener('seeked', this._video1_seeked_handler);
  video2.addEventListener('play', this._video2_play_handler);
  video2.addEventListener('pause', this._video2_pause_handler);
  video2.addEventListener('seeked', this._video2_seeked_handler);
  
  // Override temporal segmenter to use synchronized duration
  if (this.temporal_segmenter) {
    var original_init = this.temporal_segmenter._init_on_success;
    this.temporal_segmenter._init_on_success = function(groupby_aid) {
      original_init.call(this, groupby_aid);
      
      // Override the media element duration for timeline
      var original_duration = this.m.duration;
      Object.defineProperty(this.m, 'duration', {
        get: function() {
          return self.sync_duration || original_duration;
        }
      });
      
      // Update timeline boundaries to use sync duration
      if (self.sync_duration > 0) {
        this.tmetadata_gtimeline_tstart = 0;
        this.tmetadata_gtimeline_tend = self.sync_duration;
        this._tmetadata_gtimeline_draw();
      }
    };
  }
  
  // Add click event listeners to video controls for better sync
  this._add_video_control_listeners(video1, video2);
  this._add_video_control_listeners(video2, video1);
  
  // Add dual video sync to VIA's keydown handler
  this._add_dual_video_keydown_sync();
}

// Add click event listeners to video controls
_via_view_annotator.prototype._add_video_control_listeners = function(video, otherVideo) {
  var self = this;
  
  // Listen for clicks on video controls
  video.addEventListener('click', function(e) {
    // Check if click is on video controls (play button, progress bar, etc.)
    var target = e.target;
    if (target.tagName === 'VIDEO' || target.closest('video')) {
      // Small delay to let the video's own event handlers run first
      setTimeout(function() {
        if (!video.paused && otherVideo.paused) {
          otherVideo.currentTime = video.currentTime;
          otherVideo.play();
        } else if (video.paused && !otherVideo.paused) {
          otherVideo.pause();
        }
      }, 10);
    }
  ...
  // Listen for input events on video controls (progress bar dragging)
  video.addEventListener('input', function(e) {
    if (e.target.type === 'range') {
      // Progress bar was dragged
      setTimeout(function() {
        otherVideo.currentTime = video.currentTime;
      }, 10);
    }
...
// Add dual video sync to VIA's existing keydown handler
_via_view_annotator.prototype._add_dual_video_keydown_sync = function() {
  var self = this;
  var video1 = this.file_annotator[0][0].file_html_element;
  var video2 = this.file_annotator[0][1].file_html_element;
  
  // Store original on_event_keydown method
  if (!this._original_on_event_keydown) {
    this._original_on_event_keydown = this._on_event_keydown;
  }
  
  // Override the on_event_keydown method to add dual video sync
  var original_method = this._on_event_keydown;
  this._on_event_keydown = function(e) {
    // Call original method first
    original_method.call(this, e);
    
    // Add dual video sync for specific keys
    if (e.key === ' ' || e.key === 'Space') {
      // Space key - play/pause sync
      setTimeout(function() {
      ...
      // Arrow keys - seek sync
      setTimeout(function() {
        video2.currentTime = video1.currentTime;
      }, 10);
    } else if (e.key === 'l' || e.key === 'r') {
      // Frame navigation sync
      setTimeout(function() {
        video2.currentTime = video1.currentTime;
      }, 10);
    }
...
_via_view_annotator.prototype._metadata_update = function() {
  ...
  // Use appropriate container based on view mode
  if (this.view_mode === _VIA_VIEW_MODE.VIDEO2) {
    this.vid_pair_annotator_container.innerHTML = '';
    this.vid_pair_annotator_container.appendChild(table);
  } else {
...
_via_view_annotator.prototype._on_event_keydown = function(e) {
  ...
  if ( this.view_mode === _VIA_VIEW_MODE.VIDEO2 ) {
    // Handle dual video mode - use first video as primary for keyboard input
    this.file_annotator[0][0]._rinput_keydown_handler(e);
    if ( this.file_annotator[0][0].selected_mid_list.length === 0 ) {
      // no spatial region is selected
      // therefore, the temporal segmenter keyhandler should further handle this event
